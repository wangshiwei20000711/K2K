...
def B_batch(x, grid, k=0, extend=True, device='cpu'):
    ...
    if k == 0:
        ...
        if (grid[:, k:-1] - grid[:, :-(k + 1)]).any==0:
            raise Exception('grid[:, k:-1] - grid[:, :-(k + 1)]==0', grid[:, k:-1], grid[:, :-(k + 1)])
        ccc=B_km1.to(torch.float64)
        if (grid[:, k + 1:] - grid[:, 1:(-k)]).any == 0:
            raise Exception('grid[:, k + 1:] - grid[:, 1:(-k)]==0', grid[:, k + 1:], grid[:, 1:(-k)])
        # for i in range(value.shape[0]):
        #     for j in range(value.shape[1]):
        #         for kk in range(value.shape[2]):
        #             if torch.isnan(value[i,j,kk].to(torch.float64)):
        #                 print(value[i,j,kk])
        #                 raise 1
    return value
...
def curve2coef(x_eval, y_eval, grid, k, device="cpu"):
    ...
    coef = torch.linalg.lstsq(mat.to(device), y_eval.unsqueeze(dim=2).to(device)).solution[:, :, 0]  # sometimes 'cuda' version may diverge
