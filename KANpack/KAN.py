...
import numpy as np
from KANpack.KANLayer import *
from KANpack.Symbolic_KANLayer import *
from KANpack.LBFGS import *
import os
import glob
...
class KAN(nn.Module):
    ...
    --------
        __init__():
            initialize a KANpack
        initialize_from_another_model():
            initialize a KANpack from another KANpack (with the same shape, but potentially different grids)
        update_grid_from_samples():
            update spline grids based on samples
        initialize_grid_from_another_model():
            initalize KANpack grids from another KANpack
        forward():
            forward
        set_mode():
        ...
        plot():
            plot the diagram of KANpack
        train():
            train KANpack
        prune():
            prune KANpack
        remove_edge():
            remove some edge of KANpack
        remove_node():
            remove some node of KANpack
        auto_symbolic():
            automatically fit all splines to be symbolic functions
        symbolic_formula():
            obtain the symbolic formula of the KANpack network
    '''
    ...
    def __init__(self, width=None, grid=3, k=3, noise_scale=0.1, noise_scale_base=0.1, base_fun=torch.nn.SiLU(), symbolic_enabled=True, bias_trainable=True, grid_eps=1.0, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True,
        ...
        Example
        -------
        # >>> model = KANpack(width=[2,5,1], grid=5, k=3)
    ...
    def initialize_from_another_model(self, another_model, x):
        ...
        Example
        -------
        # >>> model_coarse = KANpack(width=[2,5,1], grid=5, k=3)
        # >>> model_fine = KANpack(width=[2,5,1], grid=10, k=3)
    ...
    def forward(self, x):
        ...
        Example
        -------
        # >>> model = KANpack(width=[2,5,3], grid=5, k=3)
        # >>> x = torch.normal(0,1,size=(100,2))
        # >>> model(x).shape
        torch.Size([100, 3])
        '''
    ...
    def fix_symbolic(self, l, i, j, fun_name, fit_params_bool=True, a_range=(-10, 10), b_range=(-10, 10), verbose=True, random=False):
        ...
        # >>> model = KANpack(width=[2,5,1], grid=5, k=3, noise_scale=1.)
        # >>> x = torch.normal(0,1,size=(100,2))
    ...
    def lock(self, l, ids):
        ...
        Example
        -------
        # >>> model = KANpack(width=[2,3,1], grid=5, k=3, noise_scale=1.)
    ...
    def plot(self, folder="./figures", beta=3, mask=False, mode="supervised", scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None):
        ...
        # >>> model = KANpack(width=[2,3,1], grid=3, k=3, noise_scale=1.0)
        # >>> x = torch.normal(0,1,size=(100,2))
        # >>> model(x) # do a forward pass to obtain model.acts
        # >>> model.plot()
        '''
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
        ...
        if loss_fn == None:
            ...
            loss_fn_flag = 1
            loss_fn = loss_fn_eval = lambda x1, x2, y1, y2: torch.mean(((x1 - y1) ** 2 + (x2 - y2) ** 2) + torch.maximum(torch.abs(((y2-y1)-(x2-x1))/(x2-x1))*torch.sign(-torch.abs(((y2-y1)-(x2-x1))/(y2-y1))+0.3),torch.zeros_like(y1))) #惩罚项 #暂时
            ### (torch.log10(torch.abs((y2-y1)-(x2-x1))))*10**torch.round(-torch.log10(torch.abs((y2-y1)-(x2-x1)))) *torch.abs((y2-y1)-(x2-x1))*torch.sign((y2-y1)*(x2-x1)))
        elif loss_fn == 2:
            loss_fn = loss_fn_eval = lambda x1, y1: torch.mean(((x1 - y1) ** 2)) + 0.3*torch.sum(torch.abs(x1))  # L1惩罚项
        elif loss_fn == 3:
            loss_fn = loss_fn_eval = lambda x1, y1: torch.mean(((x1 - y1) ** 2)) + 0.3 * torch.sum(torch.abs(x1)**2)  # L2惩罚项
        elif loss_fn == 4:
            loss_fn = loss_fn_eval = lambda x1, y1: torch.mean(((x1 - y1) ** 2)) + 0.3 * (0.5* torch.sum(torch.abs(x1)) +(1-0.5)/2*torch.sum(torch.abs(x1) ** 2))  # EN惩罚项
        else:
        ...
        def closure():
            ...
            pred2 = self.forward(dataset['train_input'][train_id2].to(device))
            if sglr_avoid == True:
                id_ = torch.where(torch.isnan(torch.sum(pred, dim=1)) == False)[0]
                if loss_fn_flag != 1:
                    train_loss = loss_fn(pred[id_], dataset['train_label'][train_id][id_].to(device))
                else:
                    train_loss = loss_fn(pred[id_], pred2[id_], dataset['train_label'][train_id][id_].to(device), dataset['train_label'][train_id2][id_].to(device))
            else:
                if loss_fn_flag != 1:
                    ...
                    # x1 = pred
                    # x2 = pred2
                    # y1 = dataset['train_label'][train_id].to(device)
                    # y2 = dataset['train_label'][train_id2].to(device)
                    # print('test4',torch.abs((y2-y1)*(x2-x1)))
                    # print('test3.5',torch.log10(torch.abs((y2-y1)*(x2-x1))))
                    # print('test3',10**torch.round(-(torch.log(torch.abs((y2-y1)*(x2-x1))))))
                    # print('test2',- 10**torch.round(-(torch.log(torch.abs((y2-y1)*(x2-x1))))) *(y2-y1) * (x2-x1))
                    # print('test',loss_fn(pred, pred2, dataset['train_label'][train_id].to(device), dataset['train_label'][train_id2].to(device)))
                    train_loss = loss_fn(pred, pred2, dataset['train_label'][train_id].to(device), dataset['train_label'][train_id2].to(device))
        ...
        for _ in pbar:
            ...
            train_id2 = np.random.choice(dataset['train_input'].shape[0], batch_size, replace=False)
            test_id2 = np.random.choice(dataset['test_input'].shape[0], batch_size_test, replace=False)
            while np.min(np.abs(train_id-train_id2))==0:
                train_id2 = np.random.choice(dataset['train_input'].shape[0], batch_size, replace=False)
            while np.min(np.abs(test_id-test_id2))==0:
                test_id2 = np.random.choice(dataset['test_input'].shape[0], batch_size_test, replace=False)
            ...
            if loss_fn_flag!=1:
                ...
                test_loss = loss_fn_eval(self.forward(dataset['test_input'][test_id].to(device)), self.forward(dataset['test_input'][test_id2].to(device)), dataset['test_label'][test_id].to(device), dataset['test_label'][test_id2].to(device))
    ...
    def prune(self, threshold=1e-2, mode="auto", active_neurons_id=None):
        '''
        pruning KANpack on the node level. If a node has small incoming or outgoing connection, it will be pruned away.
        ...
        # >>> from utils import create_dataset
        # >>> model = KANpack(width=[2,5,1], grid=5, k=3, noise_scale=0.1, seed=0)
        # >>> f = lambda x: torch.exp(torch.sin(torch.pi*x[:,[0]]) + x[:,[1]]**2)
        # >>> dataset = create_dataset(f, n_var=2)
        # >>> model.train(dataset, opt='LBFGS', steps=50, lamb=0.01);
        # >>> model.prune()
    ...
    def symbolic_formula(self, floating_digit=2, var=None, normalizer=None, simplify=False):
        ...
        Example
        -------
        # >>> model = KANpack(width=[2,5,1], grid=5, k=3, noise_scale=0.1, seed=0, grid_eps=0.02)
        # >>> f = lambda x: torch.exp(torch.sin(torch.pi*x[:,[0]]) + x[:,[1]]**2)
        # >>> dataset = create_dataset(f, n_var=2)
        # >>> model.train(dataset, opt='LBFGS', steps=50, lamb=0.01);
        # >>> model = model.prune()
        ...
        return symbolic_acts #[ex_round(symbolic_acts[-1][i]) for i in range(len(symbolic_acts[-1]))], x0
